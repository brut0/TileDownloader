...
def main():
    nthreads = 1 #ntiles  # May be any value not greater than n_tiles
    ...
    with Parallel(n_jobs=nthreads, backend='threading') as parallel:  # or 'multiprocessing'
            fs = (delayed(download_tile)(array[i], array[i + 1], array[nthreads]) for i in range(nthreads))
