...
def get_request(name):
    ...
    while retries < 100:
        try:
            ...
            logging.warning(f'Waiting {timeout} seconds on \'{threading.current_thread().name}\' and try again ...')
...
def main():
    ...
    for i in range(nthreads):
        thread.append(threading.Thread(target=download_tile,
                                       args=(array[i], array[i + 1], array[nthreads]),
                                       name="Thread of tiles {0}-{1}".format(array[i], array[i + 1])))
        thread[i].start()
